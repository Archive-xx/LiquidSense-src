...
public final class DatePatternConverter extends LoggingEventPatternConverter {
   ...
   private DatePatternConverter(String[] options) {
      ...
      Object simpleFormat = null;

      try {
         simpleFormat = new SimpleDateFormat(pattern);
         maximumCacheValidity = CachedDateFormat.getMaximumCacheValidity(pattern);
      } catch (IllegalArgumentException var7) {
         LogLog.warn("Could not instantiate SimpleDateFormat with pattern " + patternOption, var7);
         simpleFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss,SSS");
      }

      if (options != null && options.length > 1) {
         TimeZone tz = TimeZone.getTimeZone(options[1]);
         ((DateFormat)simpleFormat).setTimeZone(tz);
      } else {
         simpleFormat = new DatePatternConverter.DefaultZoneDateFormat((DateFormat)simpleFormat);
      }

      this.df = new CachedDateFormat((DateFormat)simpleFormat, maximumCacheValidity);
   }
