...
public abstract class MixinBootstrap {
   ...
   static boolean start() {
      if (isSubsystemRegistered()) {
         if (!checkSubsystemVersion()) {
            throw new MixinInitialisationError("Mixin subsystem version " + getActiveSubsystemVersion() + " was already initialised. Cannot bootstrap version " + "0.7.10");
         } else {
            return false;
         }
      } else {
         registerSubsystem("0.7.10");
         if (!initialised) {
            initialised = true;
   ...
   private static boolean checkSubsystemVersion() {
      return "0.7.10".equals(getActiveSubsystemVersion());
   }
